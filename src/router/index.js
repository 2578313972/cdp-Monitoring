...
function getSession(to, from, next) {
    axios
        ...
        .then((res) => {
            ...
            // // console.log('前：', res.data.role_view.menus);
            /** 添加新的菜单tab页 */
            const monitorUser = {
                children: [
            ...
            const monitorConfig = {
                children: [
                    { name: '数据工程配置', path: '/dataEngineeringConfig' },
                    { name: 'cdp报表监控配置', path: '/cdpReportConfig' }
                ],
                icon: 'ios-paper',
                name: '监控配置',
                path: ''
            };
            res.data.role_view.menus = [res.data.role_view.menus[0], monitorUser, monitorConfig];


            // console.log('后：', res.data.role_view.menus);
