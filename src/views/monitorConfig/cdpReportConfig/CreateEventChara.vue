        ...
        <Row class="padding16-18" style="background: white">
            <i-col span="24" class="flex nav">
                ...
                <Select
                    ...
                    size="large"
                >
                    <Option v-for="(item,ind) in nameArr" :value="item.displayName" :key="ind">{{
                    item.displayName
                    }}</Option>
                </Select>
                </div>
                ...
                <div style="line-height: 40px;margin-left:20px;">
                    <Button type="primary" :disabled="!displayName" @click="addList">+ 添加</Button>
                </div>
            </i-col>
...
<script>
    ...
    export default {
        ...
        created() {
            this.$https.cdpReportConfig.getTraitList().then((res) => {
                this.nameArr = res.data.data;
            });
        },
        methods: {
            addList() {
                const nameArr = this.nameArr.find(item => item.displayName === this.displayName);
            ...
            changename(e) {
                console.log(this.displayName, e);
            },
            switchChange(ind) {
